...
const payrollItems = [
  ...
  { status: "upcoming", payments: [] },
  { status: "past", payments: [] },
]
  ...
  return (
    <div className="grid gap-4">
      {payrollItems.map((payrollItem) => (
        <section key={payrollItem.status}>
          <h3 className="mb-2 text-lg font-semibold capitalize text-gray-900">
            {payrollItem.status} payments
          </h3>

          {payrollItem.payments.length > 0 ? (
            <Card>
              <CardHeader>
            ...
            <Empty title={`${payrollItem.status} payment`} />
          )}
        </section>
      ))}
    </div>
